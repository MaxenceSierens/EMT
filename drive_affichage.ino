...
float Vdiv = 99.50;        // new Voltage conversion factor
...
float Cdiv = 16.90;         // Current conversion factor
...
byte paramPointers[4] = {19,21,0,7}; // Parameter pointer for each display line


//On a 22 paramètres au total mais on commence à 0 (de 0 à 21)
char* paramLabels[] = {"Vmean ","Vrms  ","Vsdef ","Vmax  ","Vmin  ","Flux  ","f (V)","Imean ","Irms  ","Isdef ","Imax  ","Imin  ","Charge","f (I) ","Preal ","S *   ","Q *   ","Pmax  ","Pmin  ","Energy","\x01  *  ","Time"};
...
//Unités Paramètres d'affichage, si on isole un de ces paramètres il faut aussi penser à isoler la zone réservée pour l'affichage de la valeur mesurée ou calculée
char* paramUnits[] = 
                                        {"V",//0_Vmean : tension moyenne 
                                         "V",//1_Vrms : valeur efficace de la tension
                                         "V",//2_Vsdef : tension d'écart type
                                        "V",//3_Vmax : tension maximale
                                        "V",//4_Vmin : tension minimale
                                         "Vs",//5_flux
                                        "Hz",//6_f(I) : fréquence de la tension 
                                        "A",//7_Imean : courant moyen
                                         "A",//8_Irms : valeur efficace du courant 
                                         "A",//9_Isdef : courant d'écart type
                                         "A",//10_Imax : courant maximal
                                        "A",//11_Imin : courant minimal
                                         "C",//12_Q : charge
                                        "Hz",//13_f(I) : fréquence du courant
                                         "W",//14_Preal : puissance réelle
                                         "VA",//15_S* : puissance apparente
                                         "var",//16_Q* : puissance réactive
                                        "W",//17_Pmax : puissance maximale
                                         "W",//18_Pmin : puissance minimale
                                         "J",//19_énergie en Joule
                                      "\x02",//20_φ : phase en carcactères spéciaux
...
void setup() {
  ...
  lcdPrintString("ShellEcoMarathon");
  lcdWrite(true, LCDlinePos[1]);       // LCD cursor second line position 0
  lcdPrintString("  ESSTIN 2016");
  for(int md=0; md<100; md++) {        // Wait 2 seconds
    muDelay(10000);
  }
  lcdWrite(true, LCDlinePos[0]);       // LCD cursor first line position 0
  lcdPrintString("    EMT_2016     ");
  lcdWrite(true, LCDlinePos[1]);       // LCD cursor second line position 0
  lcdPrintString("   JOULEMETRE  ");
  for(int md=0; md<100; md++) {        // Wait 2 seconds
    muDelay(10000);
  }
  lcdWrite(true, LCDlinePos[1]);       // LCD cursor second line position 0
...
/*** Button STOP***/
else if(buttonPushed == 0x04) {
    buttonPushed = 0;
    Flag=0; 
  }
  
  /*** Button RAZ ***/
else if (buttonPushed == 0x08 && Flag==0) {        // If "clear values" button is pressed ...
      buttonPushed = 0;
      setProperties(); // ... set parametes to zero   
      Flag=1;  
      
      lcdWrite(true, LCDlinePos[0]);       // LCD cursor first line position 0
      lcdPrintString("Remise  A  Zero  ");
      lcdWrite(true, LCDlinePos[1]); 
      lcdPrintString("  en cours ....  ");
      delay(1000);
      asm volatile ("  jmp 0"); 
  }
...
if(Flag==1) {
 ...
 else if (buttonPushed == 0x10) {   // If "parameter select up" button is pressed ...
    buttonPushed = 0;
...
ISR(ADC_vect) {
 ...
 /*** Button debouncing ***/
  ...
  /*** Time ***/
  preTimeCnt++;
  if(preTimeCnt >= 19231 && Flag==1) {        // Quand le prescaler atteint 1931 et le Flag est à 1, on inscrémente le temps
    preTimeCnt = 0;
...
void lcdTechNot(float fval, long lval, int rangeExp, int digits, const char* unit, boolean overFlow, byte pos, boolean alignR) {
  ...
  char vvArr[] = {'f','p','n',0xE4,'m',' ','k','M','G','T','P'};  // prefix array CONVERSION
  int expo;                                 // Exponent
