...
void loop() {
  ...
  if (vCnt > 1000) {   //Après 1000 conversions ADC
    ...
    paramValues[0] = ((float(primMeanVoltCopy) * Vscale / totAverage) * 4.5 )+ 5;                 //Calcul tension moyenne  - 354.76) / 16.949; - 11.71 ) / (-65.9))*10
    
    /*** Calculate values Current ***/
    paramValues[7] = ((float(primMeanCurrCopy) * Cscale /totAverage) * 35)+1;                 //calucl courant moyen - 341.33) / -41.667;    - 1.65 )/ 6.114
  ...
  // Variante de Zihou:
    ...
    //   power = paramValues[7] * paramValues[0] ;
    ...
    // unsigned long delta_time = (now - last_sample_time)/208;
    // delta_energy = power * delta_time;
    // if (delta_energy>0){
    //   paramValues[19]+=delta_energy/4.25;
  ...
  if (boucleAffichage == 0) {
    //lcd.setCursor(8,1);
    /*if(paramValues[19] >= 1000) {
      lcd.print(paramValues[19]/1000);
      lcd.setCursor(13,1);
      lcd.print(" kJ");      
    }*/
